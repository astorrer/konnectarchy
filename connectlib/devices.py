...
MAX_DEVICES = bound.MAX_DEVICES
MAX_PLUGINS = bound.MAX_PLUGINS
...
def device_ids(bus) -> list[str]:
    ...
    return bound.strings(ids, bound.MAX_DEVICES)
...
def plugin_names(bus, device_id: str) -> list[str]:
    ...
    if not names:
        try:
            ...
            names = []
    cleaned = []
    for text in bound.strings(names, bound.MAX_PLUGINS):
...
def read_device(bus, device_id: str) -> dict:
    ...
    return {
        "id": device_id,
        "name": bound.label(get_prop(bus, path, DEVICE_IFACE, "name", device_id) or device_id),
        "type": bound.label(get_prop(bus, path, DEVICE_IFACE, "type", "")),
        ...
        "battery": battery,
        "charging": charging,
        "networkType": bound.label(network_type),
